...
class Game(object):
    ...
    def check_winner(self) -> int:#NOTE with 2 completed columns/rows both player can win (in my code to prevent the risk that opponent win thanks to MyPlayer move i assume that 2 rows/columns of different player bring to lose of last player that moved)
    ...
    def __move(self, from_pos: tuple[int, int], slide: Move, player_id: int) -> bool:
        ...
        if player_id > 2:
            return False
        # Oh God, Numpy arrays
        prev_value = deepcopy(self._board) #not sufficient to copy only 1 position
        acceptable = self.__take((from_pos[1], from_pos[0]), player_id) #ok
        if acceptable:
    ...
    def __slide(self, from_pos: tuple[int, int], slide: Move) -> bool:
        ...
        # define the corners
        SIDES = [(0, 0), (0, 4), (4, 0), (4, 4)] #NOTE: 0 row, 1 column
